 ...
 * @date 2019/10/24
 */
public class _lc_242_E_IsAnagram {


  public static void main(String[] args) {
    String s ="a", t = "a";
    _lc_242_E_IsAnagram isAnagram = new _lc_242_E_IsAnagram();
    System.out.println(isAnagram.isAnagram1(s,t));
  }
...
//方法4；分别对对应的元素计数，最后比较两个map是否相同 O（n）+ O (1)
...
Map<Character,Integer> tMap = new HashMap();
  ...
  public boolean isAnagram4(String s, String t) {
    ...
    if (sChars.length < 1 || tChars.length < 1 || sChars.length != tChars.length)  return isAnagram;
    ...
    for (int i = 0; i < sChars.length; i++) { //因为 sChars.length == tChars.length，所以此时用一个即可
      ...
      else  sMap.put(sChars[i], 1);

      if (tMap.containsKey(tChars[i]))
        tMap.put(tChars[i], tMap.get(tChars[i]) + 1);
      else  tMap.put(tChars[i], 1);
    }
    return sMap.equals(tMap);
  }

//方法5: 对以一个字符串的的元素存储，最后拿第二个map判断是否相同 O（n）+ O (n)
// 算法有问题，具体再参详
